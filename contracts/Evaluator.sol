...
contract Evaluator {
    ...
    constructor(
        ERC20TD _TDERC20,
        address payable _referenceBouncerProxy
    ) public {
        TDERC20 = _TDERC20;
    ...
    function ex1_testERC721() public {
        ...
        address studentERC721Address = studentExerciceSolution[msg.sender]
            .ERC721Address();
        IERC721 studentERC721 = IERC721(studentERC721Address);

        // Check they are two different contracts
        require(
            studentERC721Address !=
                address(studentExerciceSolution[msg.sender]),
            "ERC721 and minter are the same"
        );

        // Checking balance pre minting
        uint256 evaluatorBalancePreMint = studentERC721.balanceOf(
            address(this)
        );
        uint256 minterBalancePreMint = studentERC721.balanceOf(
            address(studentExerciceSolution[msg.sender])
        );

        // Claim a token through minter
        uint256 newToken = studentExerciceSolution[msg.sender]
            .mintATokenForMe();

        // Checking balance post minting
        uint256 evaluatorBalancePostMint = studentERC721.balanceOf(
            address(this)
        );
        uint256 minterBalancePostMint = studentERC721.balanceOf(
            address(studentExerciceSolution[msg.sender])
        );

        // Check that token 1 belongs to the Evaluator
        require(
            evaluatorBalancePostMint == evaluatorBalancePreMint + 1,
            "Did not receive one token"
        );
        require(
            studentERC721.ownerOf(newToken) == address(this),
            "New token does not belong to meeee"
        );

        // Check the token was minted, not lazily transferred
        require(
            minterBalancePostMint == minterBalancePreMint,
            "Minter balance changed, did not mint"
        );
        ...
        require(
            studentERC721.balanceOf(address(this)) == evaluatorBalancePreMint,
            "Balance did not decrease after transfer"
        );
        require(
            studentERC721.ownerOf(newToken) == msg.sender,
    ...
    function ex2_generateASignature(bytes memory _signature) public {
        ...
        address signatureSender = extractAddress(
            stringToSignToGetPoint,
            _signature
        );
        require(
            tx.origin == signatureSender,
            "signature does not match tx originator"
        );
    ...
    function ex3_extractAddressFromSignature() public {
        ...
        address signatureSender = extractAddress(
            randomBytes32ToSign[nextValueStoreRank],
            associatedSignatures[nextValueStoreRank]
        );

        // Checking that student contract is able to extract the address from the signature
        address retrievedAddressByExerciceSolution = studentExerciceSolution[
            msg.sender
        ...
        require(
            signatureSender == retrievedAddressByExerciceSolution,
            "Signature not interpreted correctly"
        );
    ...
    function ex4_manageWhiteListWithSignature(
        bytes32 _aBytes32YouChose,
        bytes memory _theAssociatedSignature
    ) public {
        // Is your signature correctly formated
        address signatureSender = extractAddress(
            _aBytes32YouChose,
            _theAssociatedSignature
        );
        // Broadcaster is signer
        require(
            signatureSender == tx.origin,
            "signature does not match tx originator"
        );

        // Is the signer whitelisted
        require(
            studentExerciceSolution[msg.sender].whitelist(signatureSender),
            "originator not whitelisted"
        );
        require(
        ...
        address storedSignatureSender = extractAddress(
            randomBytes32ToSign[nextValueStoreRank],
            associatedSignatures[nextValueStoreRank]
        );
        ...
        require(
            !studentExerciceSolution[msg.sender].whitelist(
                storedSignatureSender
            ),
            "Random signer is whitelisted"
        );
        require(
            !studentExerciceSolution[msg.sender].signerIsWhitelisted(
                randomBytes32ToSign[nextValueStoreRank],
    ...
    function ex5_mintATokenWithASpecificSignature(
        bytes memory _theRequiredSignature
    ) public {
        // Retrieve ERC721 address from ExerciceSolution
        address studentERC721Address = studentExerciceSolution[msg.sender]
            .ERC721Address();
        ...
        bytes32 dataToSign = keccak256(
            abi.encodePacked(address(this), tx.origin, studentERC721Address)
        );

        // Has sender signed the correct piece of data, and extract the address
        address signatureSender = extractAddress(
            dataToSign,
            _theRequiredSignature
        );
        ...
        // Claim a token through minter
        uint256 newToken = studentExerciceSolution[msg.sender]
            .mintATokenForMeWithASignature(_theRequiredSignature);
    ...
    function ex6_deployBouncerProxyAndWhitelistYourself(
        address payable myBouncerProxy
    ) public {
        // Build bouncer proxy
        ...
        require(
            ...
            "Deployed code is different from reference"
        );

        // Check if sender is whitelisted
        require(
            localBouncer.whitelist(tx.origin),
            "Tx originator is not whitelisted"
        );
        // For fun, whitelist sender also
        require(
            localBouncer.whitelist(msg.sender),
            "Message sender is not whitelisted"
        );
    ...
    function setRandomBytes32AndSignature(
        bytes32[20] memory _randomData,
        bytes[20] memory _signatures
    ) public onlyTeachers {
        randomBytes32ToSign = _randomData;
        associatedSignatures = _signatures;
        nextValueStoreRank = 0;
        for (uint i = 0; i < 20; i++) {
            emit newRandomBytes32AndSig(
                randomBytes32ToSign[i],
                associatedSignatures[i]
            );
        }
    }
